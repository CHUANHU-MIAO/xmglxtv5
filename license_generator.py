...
def generate_key_pair():
    ...
    print(f'请将公钥内容复制到客户端的 license_manager.py 的 PUBLIC_KEY_PEM 变量中')
    return True
...
def generate_license(fingerprint, product='xinAo_project_system'):
    private_key = load_private_key()
    if not private_key:
        return None

    message = f"{fingerprint}|{product}"
...
def auto_deploy(project_path):
    ...
    license_data = generate_license(fingerprint)
    if not license_data:
        return
...
def manual_generate():
    ...
    if choice == '1':
        ...
        license_data = generate_license(fingerprint)
        if license_data:
            output_file = os.path.join(SCRIPT_DIR, f'license_{fingerprint[:8]}.key')
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(license_data, f, ensure_ascii=False, indent=2)

            print(f'\nLicense 已生成（买断制，永久有效）')
            print(f'   设备指纹: {fingerprint}')
...
def print_usage():
    print('用法:')
    print('  python license_generator.py --auto-deploy <客户系统目录>    一键授权（推荐）')
    print('  python license_generator.py --generate-keys                生成密钥对')
    print('  python license_generator.py                               进入交互菜单')
    print()
    print('一键授权示例:')
    print('  python license_generator.py --auto-deploy ./客户系统')
    print('  python license_generator.py --auto-deploy /opt/项目管理系统/客户系统')
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--generate-keys':
            ...
            if len(sys.argv) < 3:
                print('请指定客户系统目录')
                print('用法: python license_generator.py --auto-deploy <客户系统目录>')
                return
            auto_deploy(sys.argv[2])
            return

    print('='*60)
    print('  河北鑫奥项目管理系统 - License 生成工具（买断制）')
    print('='*60)
